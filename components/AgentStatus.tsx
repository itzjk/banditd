...
export default function AgentStatus({ title, steps, intervalMs = 2200 }: Props) {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="overflow-hidden rounded-2xl border border-emerald-400/30 bg-emerald-400/[0.06]"
    >
      ...
      <div className="h-0.5 w-full overflow-hidden bg-white/10">
        <div className="h-full w-1/3 animate-[slide_1.4s_ease-in-out_infinite] rounded-full bg-emerald-400/80" />
      </div>
          ...
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${
                i < index ? "bg-zinc-600" : i === index ? "bg-emerald-400" : "bg-zinc-700"
              }`}
            />
            {s}
          </li>
